/* ============================================================
   Book Tracker — shared sample data (Ukrainian)
   Loaded as a plain script; exposes window.BookData
   ============================================================ */
(function () {
  "use strict";

  // Each cover is a *designed* typographic cover — no copyrighted art.
  // tone: how the spine/cover renders. fg/bg are the cover's own colors.
  const BOOKS = [
    {
      id: "tini",
      ...
      blurb: "Гуцульська трагедія про Івана та Марічку — кохання, що сильніше за смерть."
    },
    {
      id: "misto",
      title: "Місто",
  ...
  // pattern: reads ~5 days, pauses 1-2. Some heavier days.
  const ACTIVITY = {};
  const pagePattern = [22, 18, 0, 30, 26, 14, 0, 0, 24, 31, 19, 0, 28, 22,
                       17, 0, 25, 33, 20, 0, 0, 29, 18, 24, 15, 0, 27, 30,
                       21, 0, 19, 26, 0, 0, 23, 28, 16, 31, 0, 22, 20, 0,
                       25, 18, 29, 0, 0, 24, 33, 21, 17, 0, 26, 22, 19, 0,
                       28, 24, 0, 20, 31, 18, 0, 0, 25, 29, 23, 16, 0, 27,
                       30, 21, 19, 0, 24, 0, 28, 22, 18, 26, 31, 20, 0, 25,
                       29, 17, 23, 0, 30, 24];
  for (let i = 0; i < 90; i++) {
    ACTIVITY[iso(daysAgo(i))] = pagePattern[i] || 0;
  }
  ...
  function currentStreak() {
    let streak = 0;
    for (let i = 0; i < 90; i++) {
      if ((ACTIVITY[iso(daysAgo(i))] || 0) > 0) streak++;
      else break;
    }
    return streak;
  }
  ...
  function last7() {
    ...
    for (let i = 6; i >= 0; i--) {
      const d = daysAgo(i);
      out.push({ date: iso(d), pages: ACTIVITY[iso(d)] || 0, dow: d.getDay() });
    }
    return out;
  }
  function heat(weeks) {
    // returns weeks*7 cells, oldest -> newest, ending today
    const total = weeks * 7;
    const cells = [];
  ...
  window.BookData = {
    BOOKS, NOTES, ACTIVITY, MONTHLY, INSIGHTS, TODAY,
    iso, daysAgo,
    currentStreak, pagesToday, last7, heat,
    byId: (id) => BOOKS.find((b) => b.id === id),
    byStatus: (s) => BOOKS.filter((b) => b.status === s),
    notesFor: (id) => NOTES.filter((n) => n.bookId === id)
  };
})();
